...
public class ContainerAutoPulverizer extends ContainerAutoBase {
    private final ItemStackHandlerAutoPulverizer handler;

    public ContainerAutoPulverizer() {
        handler = new ItemStackHandlerAutoPulverizer();
    }
    ...
    static class ItemStackHandlerAutoPulverizer extends ItemStackHandlerBase {
        private ItemStack recipeOutput = ItemStack.EMPTY;
        private ItemStack recipeInput = ItemStack.EMPTY;

        public ItemStackHandlerAutoPulverizer() {
            super(2);
        }
        ...
        protected void onContentsChanged(int slot) {
            ...
            if (slot == 0) {
                int index = ModJsonRecipes.recipe_pulverizer.getIndex(getInput());
                if (index != -1) {
                    recipeInput = ModJsonRecipes.recipe_pulverizer.getInput(index);
                    recipeOutput = ModJsonRecipes.recipe_pulverizer.getOutput(index);
                } else {
