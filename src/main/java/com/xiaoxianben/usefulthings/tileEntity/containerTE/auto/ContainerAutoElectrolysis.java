...
public class ContainerAutoElectrolysis extends ContainerAutoBase {
    private final ItemStackHandlerAutoElectrolysis handler;
    private final FluidTank tank;
    public int consumeFluidAmount = 1000;

    public ContainerAutoElectrolysis() {
        handler = new ItemStackHandlerAutoElectrolysis();

        tank = new FluidTank(15000) {
            @Override
            public boolean canFillFluidType(FluidStack fluid) {
                return Objects.equals(fluid.getFluid().getName(), FluidRegistry.WATER.getName());
            }
        };
    ...
    public boolean canStart() {
        ...
        // 检查是否有有效的配方
        int index = ModJsonRecipes.recipe_electrolysis.getIndex(handler.getItemInput());
        if(index == -1) {
            return false;
        }

        // 检查输出槽位是否有足够空间
        ItemStack input = ModJsonRecipes.recipe_electrolysis.getInput(index);
        if (handler.extractItemInternal(0, input.getCount(), true).getCount() != input.getCount()) {
            return false;
        }

        // 检查输入物品是否有效
        ItemStack output = ModJsonRecipes.recipe_electrolysis.getOutput(index);
